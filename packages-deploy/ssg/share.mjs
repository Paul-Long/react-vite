...
export async function deploy(distDir, domain, bucket = 'ratex.io') {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      const uploadParams = createUploadParams(fullPath, file, domain, bucket);
      await upload(s3Client, uploadParams);
    })
  );
}

function createUploadParams(fullPath, file, domain, bucket) {
