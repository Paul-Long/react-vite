    ...
    <div className="hidden lg:flex lg:gap-x-12">
      {menus.map((m) => (
        <div key={m.key} className="relative">
          <div className="group relative flex items-center gap-x-1 text-sm font-medium leading-6 text-gray-500 hover:text-white cursor-pointer">
            {m.title}
            {m.children && (
            ...
            {m.children && (
              <div className="hidden group-hover:flex absolute -left-8 top-20px pt-16px w-auto z-10 max-w-none overflow-hidden ">
                <div className="p-16px font-size-16px text-gray-500 flex flex-col gap-16px rounded-8px bg-#b7ffe10a shadow-lg ring-1 ring-gray-900/5">
                  {m.children?.map((cm) => (
                    <div
                      key={cm.key}
